...
test_that(
  "minimum example of specify_bsvarSIGN works", 
  {
    ...
    expect_identical(
      dim(spec$data_matrices$Y)[2],
      dim(spec$data_matrices$X)[2]
    )
    expect_identical(
      length(spec$data_matrices$get_data_matrices()),
      2L
    )
    expect_identical(
      dim(spec$prior$A)[2],
    ...
    expect_identical(
      class(spec$prior$get_prior()),
      "list"
    )
    expect_true(
      det(spec$prior$B_V_inv) > 0
    )
    expect_true(
      det(spec$prior$A_V_inv) > 0
    )
    expect_identical(
      class(spec$identification)[1],
      "IdentificationBSVARSIGN"
    )
    expect_identical(
      class(spec$identification$get_identification()),
      "list"
    )
    expect_identical(
      class(spec$starting_values)[1],
      "StartingValuesBSVAR"
    )
  }
)
